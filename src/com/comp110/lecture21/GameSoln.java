...
public class Game {
  ...
  Player decideWinner() {
    ...
    for(int i = 0; i < playerCount; i++) {
      if(players[i].guess < this.answer) {
        if(closest == null || players[i].guess > closest.guess) {
          closest = players[i];
        }
      }
    }
  ...
  void revealCorrectAnswer() {
    Console.println("");
    Console.println(this.cheap.brand + " is " + this.answer + "% cheaper than " + this.expensive.brand);
    Console.println("");
  }
